...
const LogoutButton: React.FC<LogoutButtonProps> = ({ children, icon, buttonVariant = 'text', sx }) => {
  ...
  const handleLogout = () => {
    ...
    showAlert({ message: Message.LOGGED_OUT_SUCCESSFULLY, type: MessageType.SUCCESS });
  ...
  return (
    <Button onClick={handleLogout} variant={buttonVariant} sx={sx} startIcon={icon || <LogoutIcon />}>
      {children || 'Logout'}
    </Button>
  );
};
